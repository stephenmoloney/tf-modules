...
resource "azurerm_storage_account" "az_storage_account" {
  count = var.az_storage_account.tf_ignore_network_rules ? 0 : 1
...
resource "azurerm_storage_account" "az_storage_account_ignore_net_rules" {
  count = var.az_storage_account.tf_ignore_network_rules ? 1 : 0
...
resource "azurerm_storage_container" "storage_containers" {
  ...
  for_each              = var.az_storage_account.tf_ignore_network_rules ? {} : var.az_storage_account.storage_containers
  storage_account_name  = azurerm_storage_account.az_storage_account[0].name
  name                  = each.value.name
  container_access_type = each.value.access_type
}


resource "azurerm_storage_container" "storage_containers_ignore_net_rules" {
  depends_on = [azurerm_storage_account.az_storage_account_ignore_net_rules]

  for_each              = var.az_storage_account.tf_ignore_network_rules ? var.az_storage_account.storage_containers : {}
  storage_account_name  = azurerm_storage_account.az_storage_account_ignore_net_rules[0].name
  name                  = each.value.name
  container_access_type = each.value.access_type
}
